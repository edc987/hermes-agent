...
class AIAgent:
        ...
        if _config_context_length is None:
            try:
                ...
                if not isinstance(_custom_providers, list):
                    _custom_providers = []
            _config_context_length = lookup_custom_provider_context_length(
                self.model, self.base_url, _custom_providers,
            )
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        try:
            ...
            # Also check custom_providers for context_length override.
            # When auxiliary.compression.provider is "auto", the compression
            # model inherits the main model's provider/endpoint, which may be
            # a custom_providers entry with an explicit context_length that
            # get_model_context_length() can't see.
            try:
                from agent.model_metadata import lookup_custom_provider_context_length
                from hermes_cli.config import load_config as _load_cp_cfg, get_compatible_custom_providers
                _cp_cfg = _load_cp_cfg()
                _cp_list = get_compatible_custom_providers(_cp_cfg)
                _cp_ctx = lookup_custom_provider_context_length(
                    aux_model, aux_base_url, _cp_list,
                )
                if _cp_ctx is not None:
                    aux_context = _cp_ctx
            except Exception:
                pass  # Best-effort — don't break the feasibility check
