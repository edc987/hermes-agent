    ...
    try:
        ...
        for _pp in list_providers():
            ...
            if _label in _known_doctor_names:
                continue
            # Separate API key vars from base-URL override vars so the health
            # check loop never picks a URL string as a Bearer token.
            _key_vars = tuple(
            ...
            if not _key_vars:
                continue  # no API key vars — skip
            _models_url = (
                (_pp.models_url or (_pp.base_url.rstrip("/") + "/models"))
                if _pp.base_url
                else None
            )
            _apikey_providers_static.append(
                (_label, _key_vars, _models_url, _base_var, True)
            )
    except Exception:
        pass
