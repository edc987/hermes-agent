...
def lookup_custom_provider_context_length(
    model: str,
    base_url: str,
    custom_providers: list,
) -> int | None:
    """Look up context_length from custom_providers config for a specific model.

    Matches by base_url first, then by model name within the provider's
    models dict.  Returns None if no match is found.

    This is used by callers that already have access to the config's
    ``custom_providers`` list and need to resolve context length for
    display or feasibility checks — places where the full
    ``get_model_context_length()`` pipeline doesn't have access to the
    config.
    """
    ...
    for entry in custom_providers:
        ...
        if entry_url and entry_url == base_url_stripped:
